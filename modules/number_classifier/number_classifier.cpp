...
//    cv::imshow("Gray" + std::to_string(i), armor.number_img);
//    std::cout << "[res]: " << result_ss.str() << std::endl;
////    cv::waitKey(1);
//    i++;
  }
    ...
    std::remove_if(
      ...
      [this](const Armor & armor) {
        ...
        if (armor.confidence < threshold || armor.number == 'N') {
         ...
         bool mismatch = false;
         if (armor.armour_type == Robot::ArmourType::Big) {
           mismatch = armor.number == 'O' || armor.number == '2' || armor.number == '3' ||
                      armor.number == '4' || armor.number == '5';
         } else if (armor.armour_type == Robot::ArmourType::Small) {
